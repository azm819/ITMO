...
int main() {
    ...
    for (size_t i = 0; i < 10; ++i) {
        test_file("atlas shrugged", randomKey(), "tests/atlas-shrugged.in"); // 2'402'943
        test_file("atlas shrugged half", randomKey(), "tests/atlas-shrugged-half.in"); // 1'186'906
        test_file("atlas shrugged quarter", randomKey(), "tests/atlas-shrugged-quarter.in"); // 683'313
        test_file("mobydick", randomKey(), "tests/mobydick.in"); // 955'165
    }

    for (size_t i = 0; i < 50; ++i) {
        test_file("hamlet", randomKey(), "tests/hamlet.in"); // 135'003
        test_file("atlas shrugged mini", randomKey(), "tests/atlas-shrugged-mini.in"); // 108'376
        test_file("hamlet mini", randomKey(), "tests/hamlet-mini.in"); // 57'190
    }

    for (size_t i = 0; i < 100; ++i) {
        test_file("the pit and the pendulum", randomKey(), "tests/the-pit-and-the-pendulum.in"); //26'930
        // test_file("atlas shrugged small", randomKey(), "tests/atlas-shrugged-small.in"); // 17'155
    }


    auto [success, tests] = test_file("it", "it", "tests/it.in"); // 34'869
    std::cout << "\ntests ran: " << tests << "\n succeded: " << success << '\n';
